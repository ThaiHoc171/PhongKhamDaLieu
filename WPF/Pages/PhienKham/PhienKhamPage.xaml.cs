...
public partial class PhienKhamPage : Page, INotifyPropertyChanged
{
	public PhienKhamPage()
	{
		InitializeComponent();
		DataContext = this;
	...
	private void SetupColumns()
	{
		...
		GridContent.Columns.Add(
			SetupDataGrid.CreateIconButtonColumn("Eye", View_Click, "Sửa"));
	}

	private async Task LoadCombobox()
	{
		var listDoctor = await _nhanvien.GetComboboxDoctor();

		if (listDoctor.Success && listDoctor.Data != null)
		{
			listDoctor.Data.Insert(0, new NameHelper
			{
				Id = 0,
				Name = "Tất cả"
			});

			cboDoctor.ItemsSource = listDoctor.Data;
	...
	private async Task LoadData()
	{
		try
		{
			...
			if (cboDoctor.SelectedValue is int doctorId && doctorId != 0)
				nhanvienid = doctorId;
