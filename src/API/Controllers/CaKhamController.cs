...
public class CaKhamController : ControllerBase
{
	...
	public async Task<ActionResult<ApiResponse<PagedResult<CaKhamListReadModel>>>> 
		List([FromQuery] DateTime ngayKham, [FromQuery] string trangThai,[FromQuery] string loaiCaKham, [FromQuery] int? nhanVienId,
		[FromQuery] int pageNumber = 1,	[FromQuery] int pageSize = 15)
	{
		var response = await _service.GetPagedAsync(ngayKham, trangThai, loaiCaKham, nhanVienId, pageNumber, pageSize);
		return Ok(response);
	}
