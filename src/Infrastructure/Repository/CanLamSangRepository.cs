...
public class CanLamSangRepository : ICanLamSangRepository
{
    ...
    public async Task<(List<CanLamSangListReadModel>, int)>
        GetPagedAsync(int page, int size, string? loaiXetNghiem, string? trangThai)
    {
        var sql =
        $@"{BaseSelectLite}
    ...
    public async Task<List<CanLamSangListReadModel>>
        GetByLoaiXetNghiemAsync(string loaiXetNghiem)
    {
        var sql =
        $@"{BaseSelectLite}
