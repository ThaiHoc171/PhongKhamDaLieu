...
public class ThuocService
{
    ...
    public async Task<PagedResult<ThuocReadModel>> DanhSachAsync(int page, int size)
    {
        var items = await _repo.GetPagedAsync(page, size);
        var total = await _repo.CountAsync();
        return new PagedResult<ThuocReadModel>
        {
            Items = items,
            TotalCount = total,
            PageNumber = page,
    ...
    public async Task<List<ThuocReadModel>> TimKiemAsync(string keyword)
    {
        return await _repo.SearchAsync(keyword);
    }
    public async Task<List<NameResponseDTO>> ComboboxAsync()
    {
