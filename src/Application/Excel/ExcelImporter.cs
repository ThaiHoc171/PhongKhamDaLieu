...
public static class ExcelImporter
{
	public static ApiResponse<ExcelImportResult<T>> Preview<T>(Stream stream, string sheet, Func<T, int, List<string>> validator) where T : new()
	{
		...
		result.SuccessRows = validData.Count;

		if (result.Errors.Any())
			return ApiResponse<ExcelImportResult<T>>
				.SuccessResponse(result, "File Excel có dữ liệu không hợp lệ");
	...
	public static ExcelImportResult<T> Import<T>(Stream stream, string sheetName) where T : new()
	{
		...
		if (sheet == null)
		{
			...
			{
				Row = 0,
				Errors = new List<string> { $"Sheet '{sheetName}' không tồn tại" }
			});
			return result;
		}
		...
		if (sheet.Dimension == null)
		{
			...
			{
				Row = 0,
				Errors = new List<string> { "Sheet không có dữ liệu" }
			});
			return result;
		}
	...
	private static object? ConvertValue(string text, Type type)
	{
		try
		{
			...
			if (type == typeof(decimal) || type == typeof(decimal?))
				return decimal.TryParse(text, out var v) ? v : null;
