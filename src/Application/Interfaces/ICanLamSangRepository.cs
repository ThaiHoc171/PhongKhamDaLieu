...
public interface ICanLamSangRepository
{
    ...
    Task<(List<CanLamSangReadListModel>, int)>GetPagedAsync(int page, int size);
    Task<(List<CanLamSangReadListModel>, int)>SearchPagedAsync(string keyword, int page, int size);
    Task<List<CanLamSangReadListModel>>GetByLoaiXetNghiemAsync(string loaiXetNghiem);
